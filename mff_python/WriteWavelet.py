...
HMdir = '/Users/jesong1126/Python27/GeoPy/data/AMale_40'
nE = 256
nC = 257 
lfmfilename = HMdir+'/Leadfield.lfm'
...
# 1% noise  
nTrialsL = 33
Left1034 = np.zeros((nC, nSamples, nTrialsL))
for j in range(nTrialsL):
    noise = np.random.normal(0, 0.01*np.std(mepL), (257, 1600)) 
...
nTrialsR = 33 
Right2215 = np.zeros((nC, nSamples, nTrialsR))
for j in range(nTrialsR):
    noise = np.random.normal(0, 0.01*np.std(mepR), (257, 1600)) 
...
LR = np.concatenate((Left1034, Right2215), axis=2) 
