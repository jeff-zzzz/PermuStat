...
eegTvector = eegT.reshape((257*1600,1))
...
ax[2,0].axhline(ut, color='black', lw=2)
ax[2,0].axhline(-ut, color='black', lw=2)
...
ax[2,1].axhline(ut, color='black', lw=2)
ax[2,1].axhline(-ut, color='black', lw=2)
ax[3,1].scatter(range(nC), nSigTime, s=1)
...
lfmfilename = '/Users/jesong1126/Documents/MATLAB/SWCM/data/Kfdm.bin'
fd = open(lfmfilename , 'rb')
data = np.fromfile(file=fd, dtype=np.dtype('double'))  
fd.close()
K = data.reshape((nD,nC)) 
K = K.T 
...
sdenMit1 = np.mean(sden, axis=2)
sdenSigma1 = np.std(sden, axis=2)
sdenT1 =  sdenMit1 / sdenSigma1 # * np.sqrt(44) 

[np.min(sdenMit1), np.max(sdenMit1)]
[np.min(sdenSigma1), np.max(sdenSigma1)]
[np.min(sdenT1), np.max(sdenT1)]

ut = 2.58 * np.std(sdenT1)
sss = (abs(sdenT1) > ut) 
 
fig = plt.figure()
im = plt.imshow(sdenMit1, cmap='seismic', aspect="auto")
plt.colorbar(im)
#fig.savefig('Source105_mean_image.png')    
plt.show()    

fig = plt.figure()
im = plt.imshow(sdenSigma1, cmap='hot', aspect="auto")
plt.colorbar(im)
plt.show()    
#fig.savefig('Source105_std_image.png')    

fig = plt.figure()
im = plt.imshow(sdenT1, cmap='seismic', aspect="auto")
plt.colorbar(im)
plt.show()    
...
sdenT1vector = sdenT1.reshape((7341*1600,1))
mu = np.mean(sdenT1vector)
sigma = np.std(sdenT1vector)
vu = mu + 2.58 * sigma 
vl = mu - 2.58 * sigma 

fig = plt.figure()
num_bins = 50
n, bins, patches = plt.hist(sdenT1vector, num_bins, normed=1, facecolor='green', alpha=0.5)
y = mlab.normpdf(bins, mu, sigma)
plt.plot(bins, y, 'r--')
plt.vlines([vl, vu], 0, 0.1, color='black', lw=2)
plt.xlim([-2, 2])
...
fig, ax = plt.subplots(4,2,figsize=(20,10))
ax[0,0].plot(msSamples, sdenMit1.T)
ax[0,0].set_xlim([msSamples[0], msSamples[-1]])
ax[0,0].set_ylabel('mean')
ax[0,0].set_title('times')
ax[1,0].plot(msSamples, sdenSigma1.T)
ax[1,0].set_xlim([msSamples[0], msSamples[-1]])
ax[1,0].set_ylabel('sdv')
ax[2,0].plot(msSamples, sdenT1.T)
ax[2,0].set_xlim([msSamples[0], msSamples[-1]])
ax[2,0].set_ylabel('Normal')
ax[2,0].axhline(ut, color='black', lw=2)
ax[2,0].axhline(-ut, color='black', lw=2)
...
ax[3,0].set_xlim([msSamples[0], msSamples[-1]])
ax[3,0].set_ylabel('Num Sig.')
ax[0,1].plot(sdenMit1)
ax[0,1].set_xlim([0, nD])
ax[0,1].set_ylabel('mean')
ax[0,1].set_title('dipoles')
ax[1,1].plot(sdenSigma1)
